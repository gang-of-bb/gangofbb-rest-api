...
var deviceHandler = module.exports = function (req, res, next) {
    var app = req.app;

    res.locals('base', '/' == app.route ? '' : app.route);

    /*
    * overiding render method
    */
    var renderer = res.render;
    res.render = function (view, model, next) {
        var renderData = getRenderData(req, view);
        if (model != null){
            if(model._layoutFile != false){
                model._layoutFile = renderData.layout;
            }
        }
        else {
            model = { _layoutFile: renderData.layout };
        }
        renderer.call(res, renderData.view, model, next);
    }

    next();

    /*
    * return data based on request, viewname and layout.
    */
    function getRenderData(req, viewName) {
        var isMobile = isMobileDevice(req);
        var layout = (viewName == 'layout' || viewName == 'layout_mobile') ? false : (isMobile ? 'layout_mobile' : 'layout');
        var view = viewName;

        if (isMobile && layout != false) {
            var guess = path.normalize(__dirname + '/../app/views/' + viewName + "_mobile.ejs");
    ...
    /*
    *   determine if client is mobile device based on user-agent
    */
